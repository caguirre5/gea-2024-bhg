...
public:
  void run() override {
    ...
    bgMusic->addComponent<SoundComponent>(SoundType::BACKGROUND, "assets/Sounds/background.wav", FMOD_LOOP_NORMAL);
  }
};
...
public:
  void run() override {
    Entity* jumpMusic = scene->createEntity("ROAREFFECT");
    jumpMusic->addComponent<SoundComponent>(SoundType::EFFECT, "assets/Sounds/monster.wav", FMOD_DEFAULT);
  }
};

class FxMusicSetupSystem2 : public SetupSystem {
public:
  void run() override {
    Entity* jumpMusic = scene->createEntity("WALLEFFECT");
    jumpMusic->addComponent<SoundComponent>(SoundType::EFFECT, "assets/Sounds/wall.wav", FMOD_DEFAULT);
  }
};
